        ...
        if (timeSpanSelected) {
            contextMenu->addAction(QIcon::fromTheme(QStringLiteral("kt-add-filters")),
                                   tr("Filter In"), this, [this](){
                                        ...
                                        // TODO: fixme padding issues
                                        const auto timeStart = data.mapXToTime(filterRect.left() - visualRect.left() - 4);
                                        const auto timeEnd = data.mapXToTime(filterRect.right() - visualRect.left() - 6);
                                        qDebug() << data.mapTimeToX(data.minTime) << data.mapTimeToX(data.maxTime)
                                            << data.mapTimeToX(data.threadStartTime) << data.mapTimeToX(data.threadEndTime)
                                            << "filter" << filterRect.left() << filterRect.right() << "vis" << visualRect.left()
                                            << "left" << data.mapTimeToX(timeStart)
                                            << (filterRect.left() - visualRect.left())
                                            << "right" << data.mapTimeToX(timeEnd)
                                            << (filterRect.right() - visualRect.left());
